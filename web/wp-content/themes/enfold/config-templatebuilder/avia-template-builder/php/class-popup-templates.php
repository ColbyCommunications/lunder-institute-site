...
if( ! class_exists( 'Avia_Popup_Templates' ) )
{
	...
	class Avia_Popup_Templates extends aviaBuilder\base\aviaPopupTemplatesPlugins
	{
		...
		protected function font_sizes_icon_switcher( array $element )
		{
			...
			$hide_default = isset( $element['hide_default'] ) ? $element['hide_default'] : false;
			...
			foreach( $id_sizes as $size => $id )
			{
				...
				if( $textfield === true && $use_textfield )
				{
					...
					if( 'default' == $size )
					{
						...
						if( ! $hide_default )
						{
							$desc .= ' ' . __( 'Hiding this text on desktop is currently not supported and will be ignored.', 'avia_framework' );
						}
					}
					else
					{
						$desc .= ' ' . __( 'Leave empty to use desktop setting.', 'avia_framework' );
					}

					if( 'default' != $size || $hide_default )
					{
